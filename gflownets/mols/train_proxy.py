...
class Dataset(_Dataset):
    ...
    def load_h5(self, path, args, test_ratio=0.1, num_examples=None):
        ...
        print(
            "split test", sum(split_bool), len(split_bool), "num examples", num_examples
        )
        self.rews = []
    ...
    def load_pkl(self, path, args, test_ratio=0.05, num_examples=None):
        ...
        if num_examples is None:
            ...
            idxs = self.test_split_rng.choice(
                len(mols), int((1 - test_ratio) * num_examples), replace=False
            )
        test_idxs = self.test_split_rng.choice(
            len(mols), int(test_ratio * num_examples), replace=False
        )
...
def main(args):
    ...
    def save_stuff():
        ...
        pickle.dump(
            [i.data.cpu().numpy() for i in best_model.parameters()],
