...
class GraphAgent(nn.Module):
    def __init__(
        ...
        self.block2emb = nn.Sequential(
            nn.Linear(nemb + nvec_1, nemb), nn.LeakyReLU(), nn.Linear(nemb, nemb)
        )
        ...
        self.global2pred = nn.Sequential(
            nn.Linear(nemb, nemb), nn.LeakyReLU(), nn.Linear(nemb, out_per_mol)
        )
    ...
    def forward(self, graph_data, vec_data=None, do_stems=True):
        ...
        if do_stems:
            stem_block_batch_idx = (
                torch.tensor(graph_data.__slices__["x"], device=out.device)[
            ...
            if self.version == "v1" or self.version == "v4":
                ...
                stem_out_cat = torch.cat(
                    [
                        out[stem_block_batch_idx],
                        graph_data.stemtypes,
                        vec_data[graph_data.stems_batch],
                    ],
                    1,
                )
...
def mol2graph(mol, mdp, floatX=torch.float, bonds=False, nblocks=False):
    ...
    if len(mol.blockidxs) == 0:
        data = Data(  # There's an extra block embedding for the empty molecule
            ...
            stems=f([(0, 0)]),
            stemtypes=f([mdp.num_stem_types]),
