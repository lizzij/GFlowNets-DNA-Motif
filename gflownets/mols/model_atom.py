    ...
    def __init__(
        ...
        if self.version < 4:
            ...
            self.convs = nn.Sequential(
                *[
                    gnn.TransformerConv(dim, dim, edge_dim=4)
...
class MolAC_GCN(nn.Module):
    ...
    def index_output_by_action(self, s, stem_o, mol_o, a):
        ...
        return stem_o[stem_slices + a[:, 1]][torch.arange(a.shape[0]), a[:, 0]] * (
            a[:, 0] >= 0
        ) + mol_o * (a[:, 0] == -1)
    ...
    def forward(
        self, graph, vec=None, do_stems=True, do_bonds=False, k=None, do_dropout=False
    ):
        return self.mpnn(
            graph, vec, do_stems=do_stems, do_bonds=do_bonds, k=k, do_dropout=do_dropout
        )
...
def mol2graph(mol, mdp, floatX=torch.float, bonds=False, nblocks=False):
    ...
    if rdmol is None:
        g = Data(
            x=torch.zeros((1, 14 + len(atomic_numbers))),
            edge_attr=torch.zeros((0, 4)),
            edge_index=torch.zeros((0, 2)).long(),
        )
    else:
        atmfeat, _, bond, bondfeat = chem.mpnn_feat(
            mol.mol, ifcoord=False, one_hot_atom=True, donor_features=False
        )
...
def mols2batch(mols, mdp):
    batch = Batch.from_data_list(mols, follow_batch=["stems", "bonds"])
    batch.to(mdp.device)
    return batch
