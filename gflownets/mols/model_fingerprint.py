...
class MFP_MLP(nn.Module):
    def __init__(self, nhid, nvec, out_per_stem, out_per_mol):
        ...
        self.s2h = nn.Linear(
            FP_CONFIG["stem_fp_len"] * FP_CONFIG["mol_fp_radiis"][0], nhid
        )  # stem
        self.b2h = nn.Linear(
            FP_CONFIG["stem_fp_len"] * FP_CONFIG["mol_fp_radiis"][0], nhid
        )  # bond

        self.h2stemp = nn.Sequential(
        ...
        self.h2molh = nn.Sequential(
            nn.Linear(nhid * 2 + nvec, nhid), act, nn.Linear(nhid, nhid)
        )
        self.molh2o = nn.Sequential(
            nn.Linear(nhid, nhid), act, nn.Linear(nhid, out_per_mol)
        )
    ...
    def forward(self, x, v):
        ...
        per_bond_molh = self.h2molh(
            torch.cat([molh[bond_batch], bondh, v[bond_batch]], 1)
        )
        # then reduce to molh
    ...
    def action_negloglikelihood(self, s, a, g, stem_o, mol_o):
        ...
        return -(
            stem_lsm[stem_slices + a[:, 1]][torch.arange(a.shape[0]), a[:, 0]]
            * (a[:, 0] >= 0)
            + mol_lsm * (a[:, 0] == -1)
        )
...
def mol2fp(mol, mdp):
    ...
    return list(
        map(torch.tensor, fpe[0](mol, non_empty=True))
    )  # mol_fp, stem_fps, jbond_fps


def mols2batch(mols, mdp):
    molx = torch.stack([i[0] for i in mols]).to(mdp.device)
    stemx = torch.cat([i[1] for i in mols], 0).to(mdp.device)
    stem_batch = torch.cat(
        [torch.ones(i[1].shape[0], dtype=torch.long) * j for j, i in enumerate(mols)]
    ).to(mdp.device)
    bondx = torch.cat([i[2] for i in mols], 0).to(mdp.device)
    bond_batch = torch.cat(
        [torch.ones(i[2].shape[0], dtype=torch.long) * j for j, i in enumerate(mols)]
    ).to(mdp.device)
    stem_slices = torch.tensor(
        np.cumsum([0] + [i[1].shape[0] for i in mols[:-1]]),
        dtype=torch.long,
        device=mdp.device,
    )
