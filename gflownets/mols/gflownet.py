...
class Dataset:
    ...
    def _get(self, i, dset):
        if (
            self.sampling_model_prob > 0
            and self.train_rng.uniform()  # don't sample if we don't have to
            < self.sampling_model_prob
        ) or len(dset) < 32:
    ...
    def _get_sample_model(self):
        ...
        for t in range(max_blocks):
            ...
            if (
                ...
                and self.train_rng.uniform() < self.random_action_prob
            ):
    ...
    def _add_mol_to_online(self, r, m, inflow):
        if self.replay_mode == "online":
            ...
            if (
                len(self.online_mols) < self.max_online_mols
                or r > self.online_mols[0][0]
            ):
    ...
    def sample(self, n):
        if self.replay_mode == "dataset":
            ...
            if not len(self.online_mols):
                ...
                eidx = self.train_rng.choice(
                    len(self.online_mols), n, False, prio / prio.sum()
                )
                samples = sum((self._get(i, self.online_mols) for i in eidx), [])
        return zip(*samples)
    ...
    def sample2batch(self, mb):
        ...
        p_batch = torch.tensor(
            sum([[i] * len(p) for i, p in enumerate(p)], []), device=self._device
        ).long()
