...
parser.add_argument(
    "--save_path", default="results/example_branincurrin.pkl.gz", type=str
)
...
class GridEnv:
    ...
    def obs(self, s=None):
        ...
        if self.obs_type == "one-hot":
            z = np.zeros(
                (self.horizon * self.ndim + self.num_cond_dim), dtype=np.float32
            )
    ...
    def s2r(self, s):
        x = self.s2x(s)
        return (
            self.coefficients * np.array([i(x) for i in self.funcs])
        ).sum() ** self.temperature

    def reset(self, coefs=None, temp=None):
        self._state = np.int32([0] * self.ndim)
        self._step = 0
        self.coefficients = (
            np.random.dirichlet([1.5] * len(self.funcs)) if coefs is None else coefs
        )
    ...
    def state_info(self):
        ...
        s = (
            pos / (self.horizon - 1) * (self.xspace[-1] - self.xspace[0])
            + self.xspace[0]
        )
...
class FlowNet_TBAgent:
    def __init__(self, args, envs):
        self.model = make_mlp(
            [envs[0].num_obs_dim + envs[0].num_cond_dim]
            + [args.n_hid] * args.n_layers
            + [args.ndim + 1]
        )
        self.Z = make_mlp(
            [envs[0].num_cond_dim] + [args.n_hid // 2] * args.n_layers + [1]
        )
    ...
    def sample_many(self, mbsize):
        ...
        while not all(done):
            ...
            acts = cat.sample()
            ridx = torch.tensor(
                (np.random.uniform(0, 1, acts.shape[0]) < 0.01).nonzero()[0]
            )
            if len(ridx):
            ...
            for i, (bi, lp, (_, r, d, sp)) in enumerate(
                zip(np.nonzero(np.logical_not(done))[0], logp, step)
            ):
...
def compute_exact_dag_distribution(envs, agent, args):
    ...
    asd = tqdm(
        total=env.horizon ** env.ndim, disable=not args.progress or 1, leave=False
    )
    while len(stack):
        asd.update(1)
        s = stack.pop(0)
        ...
        policy = (
            softmax(
                agent.forward_logits(torch.tensor(np.float32([i.obs(s) for i in envs])))
            )
            .detach()
            .numpy()
        )
    ...
    all_int_states = np.int32(
        list(itertools.product(*[list(range(env.horizon))] * env.ndim))
    )
...
def worker(args, agent, events, outq):
    ...
    agent.envs = [
        GridEnv(args.horizon, args.ndim, funcs=agent.envs[0].funcs) for i in range(mbs)
    ]
...
def main(args):
    ...
    cond_confs = [
        ([a, 1 - a], temp) for a in np.linspace(0, 1, 11) for temp in [1, 2, 4, 8, 16]
    ]
    test_envs = [
        GridEnv(args.horizon, args.ndim, funcs=fs) for i in range(len(cond_confs))
    ]
    ...
    processes = [
        mp.Process(
            target=worker, args=(args, agent, (stop_event, backprop_barrier), losses_q)
        )
